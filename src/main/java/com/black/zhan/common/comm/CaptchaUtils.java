 ...
 * 生成验证码图片
 * 
 * 参考：http://blog.csdn.net/ruixue0117/article/details/22829557
 * @author jhshen
 * 
 */
public class CaptchaUtils {
	...
	public static String generateCaptcha(int captchaSize) {
		return generateCaptcha(captchaSize, CAPTCHAS);
	}
	...
	public static String outputVerifyImage(int w, int h, File outputFile,
			int captchaSize) throws IOException {
		...
		for (int i = 0; i < captchaSize; i++) {
			...
			affine.setToRotation(
					Math.PI / 4 * rand.nextDouble()
							* (rand.nextBoolean() ? 1 : -1), (w / captchaSize)
							* i + fontSize / 2, h / 2);
			g2.setTransform(affine);
			g2.drawChars(chars, i, 1, ((w - 10) / captchaSize) * i + 5, h / 2
					+ fontSize / 2 - 10);
		}
